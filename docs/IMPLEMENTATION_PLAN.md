...
- `#28` bis `#32` sind umgesetzt: Buchungsmaske reduziert, Bestand vor die Buchungsdaten gezogen, Ort-Editor auf `Slot hinzufuegen` umgestellt, Einstellungen-Tab eingefuehrt, Kamera-Fokus verbessert und Einheitenliste neu gestaltet. Danach wurde die Buchung weiter gestrafft: Mindestbestand ist beim schnellen neuen Artikel nicht mehr sichtbar, Ort und optionaler Slot stehen unter der Menge und die Select-Felder sind kompakter.
- Einheiten, Slot-Typen und Analyse wurden optisch weiter verdichtet: Einheit-Badges sind entfernt, Loeschen ist ein kleines Icon rechts, Slot-Typen brauchen weniger Hoehe und Bewegungen werden als horizontale Balken gezeigt.
- Die Buchung ist nachsortiert: Barcode folgt direkt auf den Artikelnamen, Chargencode und optionales Ablaufdatum stehen unter der optionalen Slot-Wahl, der Ablauf-Schalter beim schnellen neuen Artikel ist entfernt und ohne eingegebenes Ablaufdatum wird der neue Artikel ohne Ablauf gespeichert. Im Dashboard stehen Ablauf-Filter und niedrige Bestaende vor den weiteren Kennzahlen.
